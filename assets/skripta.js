...
(function () {
  ...
  function objasnjenje(sati) {
    if (sati <= 5) {
      return 'Do pet sati obračun ide po ' + CENA_SAT + ' evra za svaki započeti sat.';
    }
    if (sati <= 10) {
      return 'Od šestog do desetog sata cena je fiksnih ' + CEO_DAN +
             ' evra, bez obzira na to da li rad traje šest ili deset sati.';
    }
    var preko = sati - 10;
    return 'Prvih deset sati je ' + CEO_DAN + ' evra, pa još ' + preko +
  ...
  function osvezi() {
    ...
    var dolazak = zona ? parseInt(zona.value, 10) : 0;
    ...
    izlazDolazak.textContent = dolazak === 0 ? 'ne naplaćuje se' : evra(dolazak);
  ...
  if (kalkSekcija && trakaTekst) {
    var proveriProlaz = function () {
      var prosao = kalkSekcija.getBoundingClientRect().bottom < 0;
      if (prosao !== kalkProsao) { kalkProsao = prosao; osveziTraku(); }
    };
    window.addEventListener('scroll', proveriProlaz, { passive: true });
    window.addEventListener('resize', proveriProlaz, { passive: true });
    proveriProlaz();
  }
  ...
  if (kadar && !mirno) {
    ...
    kadar.src = putanja(1);
    var ucitaj = function () {
      for (var i = 2; i <= UKUPNO; i++) { var s = new Image(); s.src = putanja(i); }
    };
    if (window.requestIdleCallback) { requestIdleCallback(ucitaj); }
    else { setTimeout(ucitaj, 900); }
    ...
    var osveziKadar = function () {
      ...
      if (idx !== trenutni) {
        trenutni = idx;
        kadar.src = putanja(idx);
      }
    };
    izmeri();
    window.addEventListener('scroll', osveziKadar, { passive: true });
  ...
  if (zaglavlje) {
    ...
    window.addEventListener('scroll', proveriZaglavlje, { passive: true });
    proveriZaglavlje();
  }
  ...
  if (lupa && typeof lupa.showModal === 'function') {
    ...
    lupa.addEventListener('close', function () { lupaSlika.src = ''; });
  }
  ...
  if (galerija && gvTacke && gvTekuca) {
    ...
    var osveziVodic = function () {
      if (!snimci.length) { return; }
      var sirina = snimci[0].offsetWidth + 12;
